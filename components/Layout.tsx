...
const Layout: React.FC<LayoutProps> = ({ children, currentTab, setTab, user, onLogout }) => {
    ...
    return (
      <button 
        ...
        className={`flex-1 flex flex-col items-center justify-center py-2 transition-colors relative group ${
          isActive ? 'text-palette-mustard dark:text-palette-yellow' : 'text-palette-brown/50 dark:text-palette-cream/40'
        }`}
      >
        {isActive && (
            <span className="absolute top-0 w-8 h-0.5 bg-palette-mustard dark:bg-palette-yellow rounded-b-full shadow-[0_0_8px_rgba(217,141,40,0.5)]"></span>
        )}
        <div className={`p-1 rounded-full transition-all ${isActive ? 'bg-palette-cream/50 dark:bg-palette-mustard/20 translate-y-0.5' : 'group-active:scale-95'}`}>
  ...
  return (
    ...
    <div className="flex h-screen w-full bg-palette-creamLight dark:bg-palette-navy text-palette-brown dark:text-palette-cream overflow-hidden font-sans transition-colors duration-300 fixed inset-0 supports-[height:100dvh]:h-[100dvh]">
      ...
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex w-20 bg-white dark:bg-palette-navyLight border-r border-slate-200 dark:border-white/5 flex-col items-center py-6 z-50 shadow-sm transition-colors duration-300 overflow-visible">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative h-full">
        ...
        {/* Top Header */}
        <header className="bg-white/80 dark:bg-palette-navyLight/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 h-16 flex justify-between items-center px-4 md:px-8 sticky top-0 z-10 transition-colors duration-300 shrink-0">
        ...
        {/* Content Scroll Area */}
        {/* UPDATED: Increased padding bottom (pb-40) to prevent bottom nav collision on mobile */}
        <main className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth pb-40 md:pb-8 overscroll-contain">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-palette-navyLight border-t border-slate-200 dark:border-white/5 flex justify-around items-end px-2 pb-safe z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.2)]">
